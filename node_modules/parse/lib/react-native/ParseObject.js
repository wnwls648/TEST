      ...
      return query.find(options).then(async objects => {
        ...
        for (let i = 0; i < objs.length; i++) {
          ...
          if (!obj || !obj.id || !idMap[obj.id]) {
            if (forceFetch) {
              return Promise.reject(new ParseError(ParseError.OBJECT_NOT_FOUND, 'All objects must exist on the server.'));
            }
          }
        }
    ...
    if (Array.isArray(target)) {
      ...
      return filesSaved.then(() => {
        ...
        return continueWhile(() => {
          ...
          if (batch.length < 1) {
            return Promise.reject(new ParseError(ParseError.OTHER_CAUSE, 'Tried to save a batch with a cycle.'));
              ...
              return batchReturned.then((responses, status) => {
                if (responses[index].hasOwnProperty('success')) {
                  ...
                  if (!objectError && responses[index].hasOwnProperty('error')) {
                    ...
                    objectError = new ParseError(serverError.code, serverError.error); // Cancel the rest of the save
      ...
      return stateController.enqueueTask(target._getStateIdentifier(), function () {
